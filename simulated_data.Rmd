...
beta_true = as.matrix(read_csv('simulated_data/beta_true.csv'))
mu_true = as.matrix(read_csv('simulated_data/mu_true.csv'))
...
for(s in 2:niter) {
  ...
  # Update quantities for the next iteration weights
...
for(i in 1:niter){
  z[i,] <- as.vector(chains$z[[i]])
}

z_true <- as.matrix(c(rep(1,13), rep(2,13), rep(3,14)))
